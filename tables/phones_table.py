...
class PhonesTable(DbTable):
    ...
    def check_number(self, pid, tel):
        cur = self.dbconn.conn.cursor()
        cur.execute(f'SELECT phone FROM {self.table_name()} WHERE person_id =%(id)s', {'id': int(pid)})
        result = cur.fetchall()
        for i in result:
            if str(tel) == i[0]:
                return True
    ...
    def update_phone(self, tel, new_tel):
        cur = self.dbconn.conn.cursor()
        cur.execute(
            f"UPDATE {self.table_name()} SET phone=%(new_tel)s WHERE person_id={self.primary_key()[0]} AND phone=%(tel)s",
